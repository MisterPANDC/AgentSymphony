defmodule SymphonyElixir.Store.WorkflowTest do
  ...
  test "allows canceled issues to return to backlog or todo" do
    ...
    assert {:ok, _canceled} = Store.transition_workflow(todo_issue.id, "canceled", source: "user_ui", reason: "no longer needed")
    assert {:ok, todo} = Store.transition_workflow(todo_issue.id, "todo", source: "user_ui", reason: "restore as ready")
    assert todo.status == "todo"
  end
  ...
  test "local repository path stays attached to the matching project" do
    first = Store.upsert_project(project_attrs(42, "group/project-one", "Project One"))
    ...
    assert {:ok, cleared} = Store.put_project_local_repo_path(first.id, "")
