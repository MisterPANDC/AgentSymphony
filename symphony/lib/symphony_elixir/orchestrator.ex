defmodule SymphonyElixir.Orchestrator do
  @moduledoc """
  Polls the GitLab-backed read model and dispatches repository copies to Codex-backed workers.
  """
  ...
  def handle_info({:worker_runtime_info, issue_id, runtime_info}, %{running: running} = state)
    ...
    case Map.get(running, issue_id) do
      ...
      running_entry ->
        ...
        persist_run_runtime_info(updated_running_entry, runtime_info)
        notify_dashboard()
  ...
  def handle_info(
    ...
    case Map.get(running, issue_id) do
      ...
      running_entry ->
        ...
        persist_run_codex_update(updated_running_entry, update)
  ...
  defp maybe_dispatch(%State{} = state) do
    ...
    with :ok <- Config.validate!(),
      ...
      {:error, reason} ->
        Logger.error("Failed to fetch GitLab issue read model: #{inspect(reason)}")
        state
  ...
  defp block_issue_from_entry(%State{} = state, issue_id, running_entry, error) do
    persist_issue_blocked(issue_id, running_entry, error)
  ...
  defp spawn_issue_on_worker_host(%State{} = state, issue, attempt, recipient, worker_host) do
    case Task.Supervisor.start_child(SymphonyElixir.TaskSupervisor, fn ->
      ...
      {:ok, pid} ->
        ...
        run = persist_run_started(issue, attempt, worker_host)
  ...
  defp persist_run_started(%Issue{} = issue, attempt, worker_host) do
    ...
    case Store.create_run(issue.id, %{
           status: "running",
           mode: "workflow",
           started_at: DateTime.utc_now(),
           workspace_path: nil,
           summary: if(is_integer(attempt), do: "retry attempt #{attempt}", else: nil)
         }) do
      {:ok, run} ->
        Store.add_run_event(run.id, "queued", "Agent run queued", %{worker_host: worker_host})
        Store.add_run_event(run.id, "codex_started", "Agent worker started", %{worker_host: worker_host})
        run
  ...
  defp persist_run_runtime_info(%{run_id: run_id}, runtime_info) when is_binary(run_id) do
    Store.update_run(run_id, %{
      workspace_path: runtime_info[:workspace_path],
      last_heartbeat_at: DateTime.utc_now()
    })

    Store.add_run_event(run_id, "workspace_created", "Workspace ready", runtime_info)
  rescue
    _ -> :ok
  end

  defp persist_run_runtime_info(_running_entry, _runtime_info), do: :ok

  defp persist_run_codex_update(%{run_id: run_id} = running_entry, %{event: event} = update)
    ...
    event_type =
      case event do
        ...
        :turn_failed -> "failed"
        :turn_cancelled -> "canceled"
        :turn_input_required -> "operator_input_required"
        :approval_required -> "operator_input_required"
        _ -> "heartbeat"
      end

    Store.update_run(run_id, %{
      last_heartbeat_at: DateTime.utc_now(),
      codex_thread_id: Map.get(update, :thread_id) || Map.get(running_entry, :session_id)
    })

    Store.add_run_event(run_id, event_type, to_string(event), Map.drop(update, [:raw]))
  rescue
    _ -> :ok
  end

  defp persist_run_codex_update(_running_entry, _update), do: :ok

  defp persist_issue_blocked(issue_id, running_entry, error) do
    run_id = Map.get(running_entry, :run_id)
    block_type = block_type_from_running_entry(running_entry)
  ...
  defp block_type_from_running_entry(running_entry) do
    case Map.get(running_entry, :last_codex_event) do
      :approval_required -> "approval_required"
      :turn_input_required -> "operator_input"
      _ -> if codex_message_method(Map.get(running_entry, :last_codex_message)) == "mcpServer/elicitation/request", do: "mcp_elicitation", else: "external_failure"
    end
  end
